...
Jonathan Ramos
11/10/2023
Now that I've got my raw data epochs, I want to group them by treatment and keep
track of which cue was presented. There are two treatment groups of animals
(vehicle and chabc) and two cue presentation types (cocaine and saline). For
the purposes of this project, I will only consider vehicle rats. This way we can
compare directly the differences between cocaine cue presentation and saline cue
presentation.
Note that vehicle just means that the rat is not in the treatment condition group;
in otherwords the vehicle rats are our "normal" control animals.
'''
...
def main():
    ...
    for comb in itertools.product(treatment, cue, region):
        ...
        ### kind of a makeshift implementation of a flat map in python
        flattened_arr = []
        for arr in arrs:
