...
Jonathan Ramos
11/9/2023
I still have raw data which is just a 1D array of signals (2000 Hz sampling rate
for 30 minutes gives us an array of about len = 3600000 for each rat). I want
slice out +/-1 seconds worth of data around each cue presentation timestamp.
Note that with a sampling rate of fs=2000, 2 seconds is equal to 4000 samples.
I will also remove artifactual trials identified by an rms threshold.
'''
...
def rms_artifact_ID(trial, stepsize, sampsize, rmsthresh):
    '''
    takes a trial and computes rms in a sliding window fashion. If any of the
    computed rms values exceed the given rms threshold, the trial is considered
    artifactual and will be removed from all subsequent analyses.
    '''
    rmss = []
